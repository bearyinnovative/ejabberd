...
-export([
     ...
     add_room_user/4, delete_room_user/4
	]).
...
commands() ->
    [
     ...
     #ejabberd_commands{name = add_room_user, tags = [muc_room],
			desc = "Add user to a MUC room",
			module = ?MODULE, function = add_room_user,
			args = [{user, binary}, {name, binary}, {service, binary}, {host, binary}],
			result = {res, rescode}},
     #ejabberd_commands{name = delete_room_user, tags = [muc_room],
			desc = "Delete user from a MUC room",
			module = ?MODULE, function = delete_room_user,
			args = [{user, binary}, {name, binary}, {service, binary}, {host, binary}],
			result = {res, rescode}}
    ].
...
%% User是jid prefix
%% Name是room prefix
%% Service是room service
%% Host是jid host
%% jid = User@Host
%% room_jid = Name@Service.Host
add_room_user(User, Name, Service, Host) ->
    %% attrs 提取了Lang, captcha_succeed的时候SID
    %% children check_password中用到了, count_stanza_shift
    %% 整个packet。make_error_reply的时候, add_user_presence

    %io:format("~w, ~w, ~w\n", [User, Name, Service]),

    Room = <<Name/binary, "@", Service/binary, ".", Host/binary>>,
    UserJid = #jid{user=User, server=Host},
    ...
    %% Service只需要conference就够了，不用conference.localhost
    % get_room_pid只能拿到online room
    Pid = get_room_pid(Name, Service),

    {ok, StateData} = gen_fsm:sync_send_all_state_event(Pid, get_state),


    %% mod_muc_room::add_new_user(FromJid, Nick, Packet, StateData).
    %mod_muc_room::process_presence(FromJid, Nick, Packet, StateData).
    
    %% TODO: 判断成功失败
    Ret = mod_muc_room:normal_state({route, UserJid, User, Packet}, StateData),
    io:format("~w\n", [Ret]),
    ok.

delete_room_user(User, Name, Service, Host) ->
    %% TODO: 跟add_room_user只有一个字段差别，暂时容忍冗余代码
    Room = <<Name/binary, "@", Service/binary, ".", Host/binary>>,
    UserJid = #jid{user=User, server=Host},
    Packet = #xmlel{name = <<"presence">>, 
                    attrs = [{<<"from">>, UserJid}, 
