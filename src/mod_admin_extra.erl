...
commands() ->
    ...
    [
     ...
     #ejabberd_commands{name = privacy_set, tags = [stanza],
			...
			result = {res, rescode}},
     #ejabberd_commands{name = push_iq_set, tags = [stanza],
			desc = "Push a IQ set to a local or remote bare of full JID",
			module = ?MODULE, function = push_iq_set,
			args = [{to, binary}, {body, binary}],
			result = {res, rescode}},
    #ejabberd_commands{name = send_message_room, tags = [stanza],
			desc = "Send a group message to a local or remote bare of full JID of Room",
			module = ?MODULE, function = send_message_room,
			args = [{from, binary}, {to, binary}, {body, binary}],
			result = {res, rescode}},
...
%% 这个函数有个问题，如果From不是To(room)的online_user，会发送失败。
%% 除非，room不限制no paticipant也可以发消息
send_message_room(From, To, Body) ->
    Packet = {xmlel,
              <<"message">>,
              [{<<"type">>, <<"groupchat">>}],
              [{xmlel,
